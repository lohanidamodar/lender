...
class BorrowedTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if(UserBlocProvider.of(context).user == null) return CircularProgressIndicator();
        ...
        if(snapshot.connectionState == ConnectionState.active) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (BuildContext context, int i){
              ItemModel item =ItemModel.fromDocumentSnapshot(documents[i]);
              return ItemWidget(item: item, type: 'borrowed',);
            },
          );
        }else{
